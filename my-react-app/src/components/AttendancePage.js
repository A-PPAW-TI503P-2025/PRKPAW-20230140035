...
function AttendancePage() {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:3001/api/presensi/check-in", // <--- PERBAIKAN URL
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:3001/api/presensi/check-out", // <--- PERBAIKAN URL
        {},
        config
      );
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center pt-10 pb-10">
      ...
      {isLoading ? (
        ...
        <div className="bg-white p-4 rounded-lg shadow-md w-full mb-8 px-8 max-w-6xl">
          ...
          {/* FIX: Conditional rendering untuk MapContainer */}
          {coords ? (
            ...
            <div className="p-4 bg-red-100 text-red-700 rounded">
                {error || "Lokasi tidak tersedia. Silakan cek izin browser Anda."}
            </div>
          )}
        </div>
      )}
